...
from .utils import (FALSE_VALUE, TRUE_VALUE, InputDict, Json, count_files_in_dir, find_unused_file_name, parse_bool_str,
                    search_and_replace)
...
COUNTER_TOOLTIP: str = ('If enabled, counts the number of files in the specified sub-directory and uses it in the '
...
class SaveImageNode:
    ...
    def save_image(
        ...
        for (batch_number, tensor) in enumerate(images):
            ...
            file_name: str = f'{prefix_batch_num}{counter_str}{suffix_batch_num}'
            file_path: str = find_unused_file_name(full_output_dir, file_name, PNG_FILE_FORMAT)
